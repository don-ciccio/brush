...
static void BuildFoliageLayers() {
    ...
    for (int i = 0; i < g_scene.foliageCount; i++) {
        ...
        // bark AND its leaves (taking only meshes[0] lost the canopy).
        int mc = 0;
        for (int m = 0; m < fl->modelCount; m++) {
            ...
            for (int sm = 0; sm < mdl->meshCount && sc < BRUSH_FOLIAGE_SUBMESHES; sm++) {
                ...
                Texture2D t = (mdl->materialCount > 0)
                    ...
                    : (Texture2D){0};
                // A submesh with no real base-colour texture carries raylib's
                // 1x1 default (white) — treat as "none" so we fall back to the
                // layer Albedo / procedural gradient instead of flat white.
